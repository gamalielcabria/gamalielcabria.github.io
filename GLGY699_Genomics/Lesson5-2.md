...
4. In **bias correction**, the output of above's `NarrowProtDBaligned_SRR0000_1.clean.fastq` will be used as input for another BLASTx but with the **WideProtDB**.
...
5. Check the tabular formatted output, the file generated by `--out`. Number of reads matches to your **target clade** is the **read abundance** to that sequence.
    - You can use that input to plot in your tree (optional).
...
You can do this through multiple ways. One and easiest is by **repeating the above process** (Step 3) but focusing on RpoB sequences only. The other is using **SingleM** to get the rpoB counts per sample.
- For a housekeeping gene, you can just create a single database using all reviewed rpoB from pFAM database. RpoB is highly conserved and is a **single copy gene**. Single copy genes are good normalization values as they can be use as substitute for cell count.
- I would suggest using DIAMOND BLAST as it can be more straightforward.
